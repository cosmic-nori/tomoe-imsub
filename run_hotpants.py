...
def runhotpants(sci_img, ref_img, out_dir, frame_num=11, debug=False):
    ...
    try:
        frame_num = round(sci_header['EXPTIME'] / sci_header['T_FRAME'])
    except:
        frame_num = frame_num
    ...
    cmd_hotpants_base = "hotpants -inim " + sci_img + " -tmplim " + ref_img + " -outim " + out_fitsname + " -iu " + str(iu) + " -il " + str(il) + " -ig " + str(ig) + " -ir " + str(ir)  + " -tu " + str(tu) + " -tl " +  str(tl) + " -tg " + str(tg) + " -tr " + str(tr)
